...
// 5.7 extracted the vector binding check so build-gen-clusters.js can reuse it
// rather than hold a second copy of three assertions whose failure mode is a
// silent wrong answer. scripts/lib/vectors.js states the argument.
...
function main() {
  ...
  if (resolved.vectors !== undefined) {
    const record = attachVectors({
      site, slug: resolved.vectors, docs, repoRoot: REPO_ROOT, fail, log: console.log
    });
